...
public class AppStartEvent extends LineageEvent {
    ...
    public List<MetadataChangeProposalWrapper> asMetadataEvents() {
        log.warn("==========> AppStartEvent.asMetadataEvents is called");
        ...
        if (this.pipelineConfig.hasPath(PLATFORM_INSTANCE_KEY)) {
            try {
                DataPlatformInstance dpi = new DataPlatformInstance()
                        .setPlatform(new DataPlatformUrn(PLATFORM_SPARK))
                        .setInstance(LineageUtils.dataPlatformInstanceUrn(PLATFORM_SPARK, this.pipelineConfig.getString(PLATFORM_INSTANCE_KEY)));
